...
confidence_threshold = 0.01  # Minimum confidence for bounding boxes
...
print("Pre load_yolo_model")
if use_tiny_yolo:
    net, classes, output_layers = load_yolo_model_no_args()
else:
...
for frame in container.decode(stream):
    ...
    if data_gp['ts'] > 0 and data_pts['ts'] > 0:
        ...
        # # Generate AoI mask
        # frame_cv = apply_gaze_mask(frame_cv, (gaze_x, gaze_y), alpha)

        # # Generate bounding boxes
        # bounding_boxes = generate_bounding_boxes(frame_cv)

        # Detect objects using YOLO
        bounding_boxes, class_ids, confidences = detect_objects_yolo(frame_cv, confidence_threshold)

        # Apply mask with bounding boxes
        frame_cv = apply_gaze_mask_with_bounding_boxes(frame_cv, (gaze_x, gaze_y), alpha, bounding_boxes, class_ids, classes, confidences, confidence_threshold)
    ...
    # Display the resulting frame_cv
    cv2.imshow('Tobii Pro Glasses 2 - Live Scene',frame_cv)
